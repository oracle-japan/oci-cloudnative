resource "oci_identity_dynamic_group" "mushop_dynamic_group" {
  ...
  matching_rule = format(
    "Any {All {resource.type = 'devopsrepository', resource.compartment.id = '%s'}, All {resource.type = 'devopsbuildpipeline', resource.compartment.id = '%s'}, All {resource.type = 'devopsdeploypipeline', resource.compartment.id = '%s'}, All {resource.type = 'ApiGateway', resource.compartment.id = '%s'}}",
...
resource "oci_identity_policy" "mushop_policy" {
  ...
  name           = "mushop-policy"
  statements = [
    format("allow dynamic-group %s to manage devops-family in tenancy", oci_identity_dynamic_group.mushop_dynamic_group.name),
